...
// gen_non_caps : génération des coups de non-capture
void gen_non_caps()
{
	int i, j, n;
	...
	for (i = 0; i < 64; ++i)
		if (color[i] == side) {
			if (piece[i] == PAWN) {
				if (side == LIGHT) {
					/*if (COL(i) != 0 && color[i - 9] == DARK) CAPTURE
						gen_push(i, i - 9, 17);
					if (COL(i) != 7 && color[i - 7] == DARK)
						gen_push(i, i - 7, 17);*/
					if (color[i - 8] == EMPTY) {
					...
					/*if (COL(i) != 0 && color[i + 7] == LIGHT) CAPTURE
						gen_push(i, i + 7, 17);
					if (COL(i) != 7 && color[i + 9] == LIGHT)
						gen_push(i, i + 9, 17);*/
					if (color[i + 8] == EMPTY) {
...
BOOL makemove(move_bytes m)
{
	...
	set_hash(); // pas optimal car parcours 64 case
	return TRUE;
}
...
move getMove()
{
	switch ( hist_dat->moveState)
	{
	GEN_CAPS :
		gen_caps();
		hist_dat->moveState = GET_CAPS;
		hist_dat->count = first_move[ply];

	GET_CAPS:
		if (hist_dat->count < first_move[ply + 1])
		{
			return gen_dat[hist_dat->count++].m;
		}
	GEN_NON_CAPS:
		gen_non_caps();
		hist_dat->moveState = GET_NON_CAPS;
		hist_dat->count = first_move[ply];
