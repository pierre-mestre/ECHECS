...
BOOL makemove(move_bytes m)
{
...
#ifdef USE_PIECE_LIST
		// On bouge la tour
		hash ^= hash_piece[color[from]][piece[from]][from];  // retire hash de la pièce qui existait dans le hash
		hash ^= hash_piece[color[to]][piece[to]][to];		 // ajout du hash de la pièce dans le hash

		ASSERT(hash == get_hash()); //Etape de validation DM2
	...
	if (ep != -1) // un coup en passant a été joué
		hash ^= hash_ep[ep];
	if (m.bits & 8) {
		if (side == LIGHT)
			ep = m.to + 8;
	...
	/* move the piece : promotion ou bouge juste */
	color[(int)m.to] = side;
	if (m.bits & 32)
	...
	hash ^= hash_piece[color[m.to]][piece[(int)m.to]][m.to];
	hash ^= hash_piece[color[m.from]][piece[(int)m.from]][m.from];
	...
	if (m.bits & 4) {
		if (side == LIGHT) {
			// si coup en passant en avançant vers le bas
			hash ^= hash_piece[color[m.to]+8][piece[(int)m.to+8]][m.to+8];
			color[m.to + 8] = EMPTY;
			piece[m.to + 8] = EMPTY;
#ifdef USE_PIECE_LIST
			ASSERT(board[m.to + 8]);
			...
			// si coup en passant en avançant vers le haut
			hash ^= hash_piece[color[m.to] - 8][piece[(int)m.to - 8]][m.to - 8];
			color[m.to - 8] = EMPTY;
			piece[m.to - 8] = EMPTY;
#ifdef USE_PIECE_LIST
